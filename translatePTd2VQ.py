...
# import Rheo_forward_LMtest1 as RF
#####################  MODULES   ######################################################   


def convaspect(ifile,ofile,model,freq,N,RFmodule="Rheo_forward",gsopt=0):
	RF = __import__(RFmodule)
	MODEL = readaspectfile(ifile)
	MODEL['P'][ MODEL['P'] < 0 ] = 0
	
# 	for ii in range(0,3):
# 		print	 RF.PTd_VsQ(freq, MODEL['P'][ii], MODEL['T'][ii], MODEL['gs'][ii], MODEL['rho'][ii], model, 1, MODEL['Vs'][ii],constfileGlob)
...
# 		print ii
		if ii%(N/50)==1: 
			print ii,"of",N,"=",100*ii/N,"%",time.time()-t
...
# 		else:
		if gsopt==0:
			Vsi, Qi = RF.PTd_VsQ(freq,MODEL['P'][ii],MODEL['T'][ii],MODEL['gs'][ii],MODEL['rho'][ii],model,1,MODEL['Vs'][ii],constfileGlob)
		else:
			Vsi, Qi = RF.PTd_VsQ(freq,MODEL['P'][ii],MODEL['T'][ii],gsopt,MODEL['rho'][ii],model,1,MODEL['Vs'][ii],constfileGlob)
...
def readaspectfile(ifile):
	"""Reads the aspect file ifile. Returns MODEL array"""
...
def usage():
	...
	print "Usage:  %s ifile ofile model freq N RFmodule gsopt" % os.path.basename(sys.argv[0])
