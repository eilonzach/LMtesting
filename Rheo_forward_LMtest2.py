...
#	If model is extended Burgers from Jackson & Faul 2010
	if model == 'JF10_eBurg':
		...
		if P > 24.3e9: # LM
			V,Delta,TR,PR,tauMR = get_lm_parms(P)
...
# 	parser.read('constants_complete_LMtest3.ini')
...
def calc_lm_V(V0,Pi):
	"""subfunction to calculate the activation volume from some reference 
	activation volume V0 at 0 GPa, where the shape of the V/V0 curve w/ depth is given
	by Yamazak and Karato 2001, and the depth is computed from the pressure empirically"""
	# get the V_by_V0 from the paper
	dat = np.genfromtxt('lmV_Yamazaki_Karato_2001/perovskite_V_V0.txt',delimiter='\t')	
	Zs = 1000*dat[:,0] # in m
	V_by_V0s = dat[:,1]

	# get the depth from the pressure - this comes from an empirical fit of the P,Z, data
	Zi = -2.169e-17*pow(Pi,2)+ 2.378e-05*Pi + 1.026e+05

	# interpolate V_by_V0s(Z) to get scaling at the apt. Z.
	V_by_V0i = np.interp(Zi,Zs,V_by_V0s)
	V = V0*V_by_V0i
	return V
